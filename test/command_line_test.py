...
import ccx_data_pipeline.command_line as command_line


def test_command_line_args_valid_flag_version():
    """Verify correct parsing of --version flag."""
    sys.argv = ["ccx-data-pipeline", "--version"]
...
def test_command_line_args_valid_config_value():
    ...
    sys.argv = ["ccx-data-pipeline", "path_to_config_file"]
    parser = command_line.parse_args()
...
def test_command_line_args_no_config_provided():
    ...
    with pytest.raises(SystemExit) as exception:
        sys.argv = ["ccx-data-pipeline"]
        command_line.ccx_data_pipeline()
...
def test_command_line_args_invalid_arg_provided(capsys):
    ...
    sys.argv = ["ccx-data-pipeline", "--config"]
    with pytest.raises(SystemExit) as exception:
        ...
        assert "usage: ccx-data-pipeline" in captured.out
